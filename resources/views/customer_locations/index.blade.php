    ...
    <div class="loc-page">
        ...
        <div class="loc-topbar">
            <div class="d-flex align-items-center gap-2">
                ...
                <button type="button" class="store-badge-btn" onclick="openStoreModal()" title="ចុចដើម្បីកំណត់ទីតាំងហាងធំ">
                    <span id="headerStoreName">{{ $storeLocation['name'] }}</span> <i class="fas fa-cog text-muted ms-1"
                        style="font-size: 10px;"></i>
                </button>
            </div>

            <div class="d-flex align-items-center gap-2">
                <button type="button" class="btn btn-sm btn-primary py-1 px-3 fw-bold shadow-sm"
                    onclick="openQuickAddMode()">
        ...
        <div class="loc-main-container">
            <div class="loc-map-pane">
                <div class="map-floating-districts">
                    ...
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('bkk', 11.5480, 104.9284)">BKK</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('toul_kork', 11.5764, 104.8925)">Toul Kork</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('chamkarmon', 11.5449, 104.9160)">Chamkarmon</button>
                    <button type="button" class="district-pill" onclick="filterDistrict('sen_sok', 11.5989, 104.8797)">Sen
                        Sok</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('meanchey', 11.5250, 104.9130)">Meanchey</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('daun_penh', 11.5690, 104.9250)">Daun Penh</button>
                    <button type="button" class="district-pill" onclick="filterDistrict('7_makara', 11.5610, 104.9120)">7
                        Makara</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('chroy_changvar', 11.5830, 104.9310)">Chroy Changvar</button>
                    <button type="button" class="district-pill"
                        onclick="filterDistrict('toul_tompoung', 11.5360, 104.9140)">Toul Tompoung</button>
                    <button type="button" class="district-pill" onclick="filterDistrict('takhmao', 11.4780, 104.9510)">Ta
                        Khmau</button>
                </div>
                ...
                <div id="storeRelocateAlert" class="store-relocate-alert">
                    <span>🍕 ទីតាំងហាងថ្មី៖ <span id="tempStoreCoordText" class="text-primary">11.5564,
                            104.9282</span></span>
                    <button type="button" class="btn btn-xs btn-primary py-1 px-2 fw-bold"
                        onclick="saveStoreCoordinatesDirectly()">
                ...
                <div id="routeInfoBadge" class="map-route-badge">
                    <div class="d-flex align-items-center gap-2">
                        <span class="text-primary fw-bold"><i class="fas fa-route me-1"></i><span id="routeDistanceText">0.0
                                km</span></span>
                        <span class="text-muted small">~<strong id="routeEstTime">0</strong> នាទី</span>
                        <a id="btnGmapsNavDirect" href="#" target="_blank"
                            class="btn btn-xs btn-outline-danger py-0 px-2 fw-bold" style="font-size: 10.5px;">
                ...
                <div class="loc-sidebar-header">
                    ...
                    <div class="input-group input-group-sm mb-2">
                        ...
                        <input type="text" id="sidebar_gmaps_paste" class="form-control border-start-0"
                            placeholder="Paste Google Maps Link / GPS..."
                            onkeydown="if(event.key==='Enter') parseSidebarGmaps()">
                    ...
                    <form method="GET" action="{{ route('customer-locations.index') }}" class="d-flex gap-1">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="ស្វែងរកអតិថិជន..."
                            class="form-control form-control-sm">
                        <select name="location_status" class="form-select form-select-sm" style="width: 105px;"
                            onchange="this.form.submit()">
                            <option value="all" {{ request('location_status') == 'all' ? 'selected' : '' }}>ទាំងអស់</option>
                            <option value="pinned" {{ request('location_status') == 'pinned' ? 'selected' : '' }}>មាន GPS
                            </option>
                            <option value="unpinned" {{ request('location_status') == 'unpinned' ? 'selected' : '' }}>គ្មាន
                                GPS</option>
                        </select>
                    ...
                    @forelse($customers as $c)
                        ...
                        <div class="loc-cust-card" id="card-cust-{{ $c->id }}" onclick="focusCustomerOnMap({{ $c->id }})">
                                ...
                                <div class="fw-bold text-dark text-truncate" style="max-width: 200px;">
                                    ...
                                    @if($c->phone)
                                        <small class="text-muted ms-1 font-monospace"
                                            id="cust-card-phone-{{ $c->id }}">{{ $c->phone }}</small>
                                    @endif
                                </div>
                                <span id="cust-card-badge-{{ $c->id }}"
                                    class="badge {{ $hasGps ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning' }}"
                            ...
                            <div class="mt-2 pt-1 border-top d-flex justify-content-between align-items-center">
                                <div class="small fw-semibold text-primary" style="font-size: 11px;"
                                    id="dist-text-{{ $c->id }}">
                                    @if($hasGps)
                                        <span class="cust-distance-val" data-lat="{{ $c->latitude }}"
                                            data-lng="{{ $c->longitude }}">— km</span>
                                    @else
                                ...
                                <div class="d-flex gap-1" id="cust-card-actions-{{ $c->id }}">
                                    @if($hasGps)
                                        <a id="cust-card-gmaps-{{ $c->id }}"
                                            href="https://www.google.com/maps/dir/?api=1&origin={{ $storeLocation['lat'] }},{{ $storeLocation['lng'] }}&destination={{ $c->latitude }},{{ $c->longitude }}&travelmode=driving"
                                            target="_blank" class="btn btn-xs btn-light border py-0 px-2 text-danger fw-bold"
                                            style="font-size: 10px;" onclick="event.stopPropagation()">
                                            <i class="fab fa-google"></i>
                                        </a>
                                    @endif
                                    <button type="button" class="btn btn-xs btn-outline-primary py-0 px-2 fw-semibold"
                                        style="font-size: 10.5px;"
                                        onclick="event.stopPropagation(); editCustomerDrawer(@js($c))">
            ...
            <div id="quickEditDrawer" class="quick-edit-drawer">
                ...
                <div id="drawerAlertBox" class="alert alert-danger py-1 px-2 small mb-2 d-none" style="font-size: 11px;">
                </div>
                ...
                <form id="drawerLocationForm" onsubmit="saveDrawerLocation(event)">
                    ...
                    <div class="mb-2">
                        ...
                        <datalist id="drawerCustList">
                            @foreach($allCustomers as $cust)
                                <option value="{{ $cust->name }} | {{ $cust->phone ?: 'No Phone' }} [#{{ $cust->id }}]">
                                </option>
                            @endforeach
                        </datalist>
                    </div>
                    ...
                    <div class="mb-2">
                        <label class="form-label small fw-bold mb-1" style="font-size: 11px;">ឈ្មោះ <span
                                class="text-danger">*</span></label>
                        <input type="text" id="drawer_name" class="form-control form-control-sm" required
                    ...
                    <div class="row g-2 mb-2">
                        <div class="col-6">
                            ...
                            <input type="text" id="drawer_phone" class="form-control form-control-sm"
                    ...
                    <div class="mb-2">
                        <label class="form-label small fw-bold mb-1" style="font-size: 11px;">ចំណាំ</label>
                        <input type="text" id="drawer_landmark" class="form-control form-control-sm"
                            placeholder="ទល់មុខសាលា, ជិតផ្សារ...">
                    </div>

                    <div class="mb-2">
                        <label class="form-label small fw-bold mb-1" style="font-size: 11px;">អាសយដ្ឋាន</label>
                        <textarea id="drawer_address" class="form-control form-control-sm" rows="2"
                            placeholder="ផ្ទះលេខ, ផ្លូវ..."></textarea>
                    </div>
                    ...
                    <div class="mb-3 p-2 bg-light rounded border">
                        ...
                        <div class="row g-1">
                            <div class="col-6">
                                <input type="text" id="drawer_lat" class="form-control form-control-sm bg-white" readonly
                                    required placeholder="Lat">
                            </div>
                            <div class="col-6">
                                <input type="text" id="drawer_lng" class="form-control form-control-sm bg-white" readonly
                                    required placeholder="Lng">
                            </div>
                        </div>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="button" class="btn btn-sm btn-light border w-50"
                            onclick="closeQuickDrawer()">បោះបង់</button>
                    ...
                    <div class="modal-body p-3">
                        ...
                        <div class="p-2 mb-3 rounded bg-light border">
                            ...
                            <div class="input-group input-group-sm">
                                <input type="text" id="store_gmaps_input" class="form-control"
                                    placeholder="https://maps.app.goo.gl/... ឬ 11.5564, 104.9282">
                        ...
                        <div class="mb-2">
                            <label class="form-label small fw-bold">ឈ្មោះហាង / សាខា <span
                                    class="text-danger">*</span></label>
                            <input type="text" name="name" id="store_name_input" class="form-control form-control-sm"
                                required value="{{ $storeLocation['name'] }}">
                        </div>

                        <div class="row g-2 mb-2">
                            <div class="col-6">
                                <label class="form-label small fw-bold">លេខទូរស័ព្ទ</label>
                                <input type="text" name="phone" id="store_phone_input" class="form-control form-control-sm"
                                    value="{{ $storeLocation['phone'] ?? '012 345 678' }}">
                            </div>
                            <div class="col-6">
                                <label class="form-label small fw-bold">អាសយដ្ឋាន</label>
                                <input type="text" name="address" id="store_address_input"
                                    class="form-control form-control-sm"
                                    value="{{ $storeLocation['address'] ?? 'Phnom Penh' }}">
                            </div>
                        </div>

                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <label class="form-label small fw-bold text-dark mb-0">កូអរដោនេ GPS ហាង៖</label>
                            <button type="button" class="btn btn-xs btn-outline-success py-0 px-2" style="font-size: 11px;"
                                onclick="useMyGpsForStore()">
                        ...
                        <div class="row g-2 mb-3">
                            <div class="col-6">
                                ...
                                <input type="number" step="any" name="lat" id="store_lat_input"
                                    class="form-control form-control-sm" required value="{{ $storeLocation['lat'] }}">
                            </div>
                            <div class="col-6">
                                <label class="form-label text-muted mb-0" style="font-size: 10px;">Lng</label>
                                <input type="number" step="any" name="lng" id="store_lng_input"
                                    class="form-control form-control-sm" required value="{{ $storeLocation['lng'] }}">
                            </div>
                        </div>

                        <div class="border-top pt-2">
                            <label class="form-label small fw-bold text-muted mb-1">រង្វង់តំបន់ដឹកជញ្ជូន (Delivery
                                Radius):</label>
                            <div class="row g-2">
                                <div class="col-4">
                                    <label class="text-muted" style="font-size: 10px;">Zone 1 (បៃតង)</label>
                                    <div class="input-group input-group-sm">
                                        <input type="number" step="0.5" name="radius_1" id="store_r1" class="form-control"
                                            value="{{ $storeLocation['radius_1'] ?? 3 }}">
                                ...
                                <div class="col-4">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <input type="number" step="0.5" name="radius_2" id="store_r2" class="form-control"
                                            value="{{ $storeLocation['radius_2'] ?? 5 }}">
                                        <span class="input-group-text">km</span>
                                    </div>
                                </div>
                                <div class="col-4">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <input type="number" step="0.5" name="radius_3" id="store_r3" class="form-control"
                                            value="{{ $storeLocation['radius_3'] ?? 8 }}">
                                        <span class="input-group-text">km</span>
                                    </div>
                                </div>
                            </div>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            window.saveStoreCoordinatesDirectly = function () {
                ...
                fetch(url, {
                    ...
                    .catch(err => {
                        alert("មានបញ្ហា ក្នុងការរក្សាទុកទីតាំងហាង!");
                        console.error(err);
                    });
            };
