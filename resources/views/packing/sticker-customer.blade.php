    ...
    <body>
        <div class="sticker" id="invoice-content">
            ...
            @php
                // ---- KHR base pricing ----
                // Use the product's actual KHR base prices, not calculated from USD
                $exchangeRate = 4000;
                ...
                // Subtotal in KHR = sum of (product price_khr * qty) for priced items.
                $subtotalKhr = $paidItems->sum(function ($item) {
                    return (float) $item->product->price_khr * (float) $item->quantity;
                });

                $discountKhr = (float) $invoice->discount_amount * $exchangeRate;

                // Grand total in KHR:
                // subtotal - discount + delivery fee (delivery fee is already stored in KHR on the invoice).
